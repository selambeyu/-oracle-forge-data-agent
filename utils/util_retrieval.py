...
before the agent attempts any query. This is the discovery-first discipline
from the OpenAI data agent — prevents overconfident wrong-table selection.

Three passes:
  Pass 1 — Dataset routing: which dataset(s) does this query touch?
  Pass 2 — Table ranking:   which tables within those datasets are relevant?
  Pass 3 — Column filtering: which columns matter for this specific query?
...
Usage:
    ...
    # result.datasets   → ["crmarenapro"]
    # result.tables     → [{"dataset": "crmarenapro", "table": "orders", "score": 0.87}, ...]
    # result.columns    → [{"table": "orders", "columns": ["customer_id", "purchase_date", "amount"]}]
...
class RetrievalResult:
    ...
    columns:     list[dict]   # [{"dataset", "table", "columns"}]  — Pass 3 placeholder
    explanation: str
...
class MultiPassRetriever:
    """
    Three-pass table discovery aligned with the OpenAI data agent
    ...
    def retrieve(
        ...
        """
        Run all three passes and return ranked candidates.

        Args:
            query:           Natural language user query
            top_datasets:    Max datasets to consider (default 3)
            top_tables:      Max tables to return (default 5)
            min_table_score: Minimum keyword overlap score to include a table

        Returns:
            RetrievalResult with datasets, tables, columns, explanation
        """
        ...
        if not table_candidates:
            warnings.append("Pass 2: No tables scored above threshold. Widen search or check KB v2.")

        # ── Pass 3: column filtering (placeholder — extend with live introspection) ──
    ...
    def explain(self, query: str) -> str:
        """Return human-readable retrieval reasoning — useful for agent traces."""
...
def _tokenise(query: str) -> list[str]:
    """Lowercase, strip punctuation, split to tokens + bigrams."""
    q = query.lower()
...
def _filter_columns(tokens: list[str], tables: list[dict]) -> list[dict]:
    """
    Pass 3: basic column filtering based on token overlap.
    Extend this to call schema_introspector for live column lists.
    """
    result = []
    for t in tables:
...
def _infer_db_type(dataset: str, table: str) -> str:
    """Best-guess db_type for a table based on dataset index."""
